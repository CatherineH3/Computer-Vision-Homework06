...
class roiPoly():
    ...
    def select_image(self):
        ...
        self.origImage = Image.open(str(filename)).convert('RGB')
         
        # New to resize
        # Get width and height of og stuff
        w, h = self.origImage.size 
        # Compute scaling factor to fix max screen size
        max_width, max_height = 1200, 800
        scale = min(max_width / w, max_height / h, 1)  # don't upscale
        new_size = (int(w*scale), int(h*scale))
        # Actually resize the image
        self.origImage = self.origImage.resize(new_size, Image.LANCZOS)
...
if __name__ == "__main__":
    ...
    # New version (scaled)
    # Scale both images down if they are too big
    ...
    scale = min(max_display_width / (w1 + w2), max_display_height / max(h1, h2), 1)
    ...
    cv2.imshow('Warped Images', np.concatenate([I_transformed_small, I_correct_small], axis=1))
    cv2.waitKey(0)
